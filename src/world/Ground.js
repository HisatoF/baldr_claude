...
function buildSurfaceGeometry(x0, x1, z0, z1, segX, segZ, lift) {
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    pos.setX(i, x);
    pos.setZ(i, z);
    pos.setY(i, surfaceHeightAt(x, z) + lift);
  }
...
export function buildGround(opts = {}) {
  ...
  const SEG_X = 320;
  const SEG_Z = 60;
  const geo = buildSurfaceGeometry(GROUND_MIN_X, GROUND_MAX_X, GROUND_MIN_Z, GROUND_MAX_Z, SEG_X, SEG_Z, 0);
...

  {
    ...
    for (let i = 0; i < 34; i++) {
      ...
      const w = 1.2 + rnd() * 2.6;
      const g = new THREE.BoxGeometry(w, 0.55 + rnd() * 0.5, 1.0 + rnd() * 0.9);
    ...
    for (const f of FEATURES) {
      ...
      for (let k = 0; k < 3; k++) {
        ...
        const z = (rnd() - 0.5) * 18;
        const g = new THREE.BoxGeometry(2.4 + rnd() * 3.2, 0.32, 2.0 + rnd() * 3.0);
